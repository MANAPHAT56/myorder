...
function ShopsPage({ notify }) {
  ...
  // --- ส่วนที่เพิ่มเข้ามาสำหรับ Blacklist ---
  ...
  // --- ปรับฟังก์ชัน Blacklist ---
  const handleExecuteBlacklist = () => {
    if (!blacklistReason.trim()) {
      setBlacklistError("กรุณาระบุเหตุผลในการ Blacklist");
      return;
    }
    
    // อัปเดตสถานะร้าน และเก็บเหตุผลไว้ (เอาไปเชื่อม API ได้)
    setShops(prev => prev.map(s => s.id===selectedShop.id ? { ...s, is_blacklisted:true, blacklist_reason: blacklistReason } : s));
    setShowBlacklistConfirm(false);
    setSelectedShop(null); // ปิดหน้าต่างจัดการร้านค้าด้วย
  ...
  return (
    <div>
      ...
      {selectedShop && (
        <Modal title={`จัดการ: ${selectedShop.name}`} onClose={() => setSelectedShop(null)} wide>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:16 }}>
            ...
            {/* เปลี่ยน onClick ของปุ่ม Blacklist ให้เปิดหน้าต่างยืนยันแทน */}
            <button className="btn btn-warn" 
              onClick={() => { setBlacklistReason(""); setBlacklistError(""); setShowBlacklistConfirm(true); }} 
              disabled={selectedShop.is_blacklisted}>
      ...
      {/* ------------------------------------------------------------------ */}
      {/* เพิ่ม Modal สำหรับกรอกเหตุผล Blacklist */}
      {showBlacklistConfirm && selectedShop && (
        <Modal title="⛔ ยืนยันการแบนร้านค้า (Blacklist)" onClose={() => setShowBlacklistConfirm(false)}
          footer={<><button className="btn btn-ghost btn-sm" onClick={() => setShowBlacklistConfirm(false)}>ยกเลิก</button><button className="btn btn-warn btn-sm" onClick={handleExecuteBlacklist}>ยืนยันการแบน</button></>}>
          
          <div className="alert alert-warn" style={{ marginBottom:16 }}>⚠️ การกระทำนี้จะปิดกั้นไม่ให้ร้านค้านี้แสดงผลในระบบทันที</div>
          <p style={{ fontSize:14, marginBottom: 12 }}>เพิ่ม <strong>"{selectedShop.name}"</strong> ลงใน Blacklist?</p>
          
          <div className="form-group">
            <label className="form-label">เหตุผลในการแบน *</label>
            <textarea 
              className={`input textarea ${blacklistError ? "error" : ""}`} 
              placeholder="ระบุเหตุผล เช่น ทุจริตการจัดส่ง, ได้รับใบเคลมเลขที่..." 
              value={blacklistReason} 
              onChange={e => { setBlacklistReason(e.target.value); setBlacklistError(null); }} 
              rows={3}
            />
...
function ClaimsPage({ notify }) {
  ...
  // ส่วนที่เพิ่มใหม่: ฟังก์ชันสำหรับ Blacklist ร้านค้า
  const handleBlacklist = (clm) => {
    if (window.confirm(`ยืนยันการระงับการใช้งานร้านค้า "${clm.shop_name}" หรือไม่?`)) {
      // ในแอปจริง ตรงนี้ต้องส่ง API ไปอัปเดตสถานะร้านค้าใน Database
      notify(`เพิ่มร้าน "${clm.shop_name}" ลงในบัญชีดำเรียบร้อยแล้ว`, "error");
      setSelected(null);
    }
  };
    ...
    <div>
      ...
      {selected && (
        <Modal title={`คำร้องเคลม: ${selected.id}`} onClose={() => setSelected(null)} wide>
          ...
          {/* ปุ่มจัดการ: แบ่งเป็น Blacklist (ซ้าย) และ Resolve (ขวา) */}
